...
private:
    size_t sz = 0;
    size_t capacity = 1;
    char* str = new char(1);// nullptr при capacity = 1? Плохое поведение
    //исправил, вроде бы // new (1) лучше, либо здесь хоть один символ впиши, чтобы размер оправдать
public:
    ...
    //чтобы сразу в память вставлять, хотя ладно, пока не знаю// Почти, он забирает у принимаемого аргумента память и прикидывается и делает так, что это
    // становится памятью нового элемента, а у старого остаётся пачка нулов
    ...
    const char& operator[] (size_t index) const {
        return str[index];
    }// возвращает константный i-ый символ
    ...
    size_t rfind(const String& subs) const{
        for(size_t i = sz; i > 0; --i){// А последний символ не проверяешь? А вдруг subs из одного символа состоит, который как раз в 0 позиции? Доделай
            if (str[i-1] == subs[0]){// поправил // Ммм, int, не, стоит использовать size_t, но со сдвигом (начинать с sz и заканчивать >0) либо условие сделать
                // в конце через break
                bool mark = true;
                int k = i;
...
//        }
    ...
    void pop_back(){
        ...
        if(4*sz <= capacity){// насколько я помню, с ним не заходило
            capacity /= 2;// Потому что кто-то опять забывает удалять массив
            char* copy = str;
